...
export function parseJsonFindings(report: string): StructuredFinding[] | null {
	...
	if (!last) return null;
	try {
		const parsed = JSON.parse(last) as { findings?: Array<Record<string, unknown>> };
		if (!Array.isArray(parsed.findings)) return null;
		const findings = parsed.findings
			.map(normalizeFinding)
			.filter((finding): finding is StructuredFinding => finding !== null);
		return findings.every((finding) => validateStructuredFinding(finding).length === 0)
			? findings
			: null;
	} catch {
		return null;
...
function normalizeFinding(
	...
	const actual = String(finding.actual ?? finding.summary ?? finding.description ?? "").trim();
	if (!actual) return null;
	const severity = FINDING_SEVERITIES.includes(finding.severity as FindingSeverity)
		? (finding.severity as FindingSeverity)
		: "major";
	const line = Number(finding.line);
	return {
		id: typeof finding.id === "string" && finding.id ? finding.id : `F${index + 1}`,
		severity,
		category: typeof finding.category === "string" && finding.category ? finding.category : "uncategorized",
		...(typeof finding.file === "string" && finding.file ? { file: finding.file } : {}),
		...(Number.isFinite(line) && line > 0 ? { line } : {}),
		...(typeof finding.task === "string" && finding.task ? { task: finding.task } : {}),
		...(typeof finding.claim === "string" && finding.claim ? { claim: finding.claim } : {}),
		...(Array.isArray(finding.evidence)
			? { evidence: finding.evidence.filter((item): item is string => typeof item === "string" && item.trim().length > 0) }
			: {}),
		actual,
	};
}
