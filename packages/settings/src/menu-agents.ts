...
/** Checkbox picker when the surface supports ui.custom; select-loop toggles
 *  otherwise. Either way the override lands as ONE validated write in tier order. */
async function toggleTiers(
	...
	while (true) {
		...
		const picked = await ui.select(
			...
			TIER_IDS.map(
				(tier) => `${current.includes(tier) ? "[x]" : "[ ]"} ${tier}`,
			),
		);
		if (!picked) return;
