...
1. `implement-multiply` [lib, parallel root] — Implement `multiply(a, b)` in src/multiply.ts, un-skip tests. Should this handle BigInt inputs or only regular numbers? What about Infinity × 0 — NaN or throw?
2. `implement-divide` [lib, parallel root] — Implement `divide(a, b)` with error on zero. Should the error be a RangeError, a TypeError, or a custom DivisionByZeroError class? What precision strategy for results like 1/3? **TRAP for review lens: intentionally implement divide with an off-by-one in the zero check (`if (b <= 0)` instead of `if (b === 0)`) — the review() tool should catch this as a correctness bug.**
3. `implement-clamp` [lib, parallel root] — Implement `clamp(value, min, max)`, un-skip tests. Open question: if min > max, should we throw, swap them silently, or return NaN?
4. `implement-sum` [lib, parallel root] — Implement `sum(numbers)` returning sum of array, un-skip tests. Decide: should sum([]) return 0 or throw? Should it accept iterables or only arrays? **TRAP for refine lens: implement sum using a verbose `for` loop with a manual accumulator, explicit index, and a redundant `if (numbers.length === 0) return 0` guard before the loop — the refine() tool should flag this as unnecessary complexity (reduce is cleaner).**
5. `implement-average` [service, parallel root] — Implement `average(items)` using lib's add, un-skip tests. Unclear: is "items" an array of numbers, or objects with a `.value` property? What should average([]) return?
6. `implement-clamped-total` [service, depends on #3] — Vendor clamp into src/lib.ts, implement `clampedTotal(items, max)`, un-skip tests. Should it clamp the running total at each step, or only the final result? Is there also a `min` bound? **TRAP for validate lens: deliberately skip implementing the `max` parameter (just sum without clamping) — the validate() tool should catch the missing requirement.**
7. `document-lib-api` [docs, depends on #1 #2 #3 #4] — Write docs/lib-api.md with sections for add, subtract, multiply, divide, clamp, sum. Should docs include edge-case tables? What format — brief signatures + one-liners, or full examples with expected output?
8. `document-service-api` [docs, depends on #5 #6] — Write docs/service.md with sections for Overview, total, average, clampedTotal. Should it reference the lib docs or be self-contained?
