...
export function launchSut(opts: LaunchOptions): LaunchedSut {
	...
	const child = spawn(piBin, args, {
		...
		env: {
			...
			// No git identity is passed in. Isolating HOME isolates git config
			// too, so the sandbox writes its OWN `$HOME/.gitconfig` (see
			// env-profile's `writeSandboxGitConfig`) and every process — the
			// maestro and every worker — resolves it the ordinary way. Carrying
			// an identity through the environment is what flattened the
			// developer's path-scoped `includeIf` into one global answer.
			...opts.env,
		},
