...
```
/plan ─▶ converge ─▶ [Shift+Tab #1] ─▶ form + preview ─▶ (stay in plan) ─▶ [Shift+Tab #2] ─▶ auto ─▶ review ─▶ rule ─▶ exec
 boot    open Qs,     no plan yet →     author tree +      review the        plan formed →     gate    plan-    human   fork+seed
