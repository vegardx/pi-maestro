...
1. **PR-1 — projection prep (optional but recommended).** Introduce a `PlanView` projection type (rows with id/depth/state/tasks) and port `hud-wiring.buildPlanView`, `dashboard.renderAgentsOverview`, and `ui.ts` renders onto it, built from v1 (`depth` always 0). Pure refactor, green, shrinks the flip PR by three consumer files.
...
5. **PR-5 — `NodeExecutor` + adapter port, unwired.** New executor over the tree; a v2-keyed copy of the adapter spawn/completion/restart path (the adapter takes the engine as a constructor arg, so the v2 variant can be instantiated only by tests). Add focused lifecycle and dirty-completion component tests before anything flips. Green.
6. **PR-6 — exec periphery.** seeds/shipper/recovery/verify/workspace-validation v2 variants + their tests. Green, unwired.
7. **PR-7 — THE FLIP.** `runtime/context.ts`, `commands.ts`, `hud-wiring`, `dashboard`, `tools.ts`, RPC protocol v7, and `renderPlanForAgent`; **delete** `schema.ts`, `engine.ts`, `deliverable-executor.ts`, `stage-runtime.ts`, workflow validation, and old tests; flip the storage default to v6 and add boot-time legacy handling (below). Big but mechanical: every semantic piece was pre-tested in PR-3…6.
...
| # | Risk | Mitigation |
| --- | --- | --- |
| R1 | **Executor/adapter rewrite regressions.** The completion lattice (task-gated workers vs idle-done read agents, `IDLE_DONE_THRESHOLD` re-fed by the 5s poll, dirty-hold steer cadence + escalation, crash-respawn caps, generation guards vs buffered RPC) encodes a year of incident fixes (task-toggle wedge, dirty-hold wedge, kill-mid-review hole). A recursive rewrite can silently drop any of them. | Port the state machines **verbatim keyed by nodeId**, not re-derive them. Define focused component parity checks before the flip PR opens. Grep-list every `logEvent` name and assert the v2 stack emits the same event vocabulary. |
| R2 | **The flip PR (PR-7) is huge** — context.ts (~2600 lines), commands, HUD, RPC, and tools change together; review fatigue is where regressions land. | PR-1 projection prep removes consumers up front; PR-3…6 make the flip *mechanical* (only re-keying and wiring, no new semantics allowed in PR-7 by review rule). Define acceptance evidence before merging the flip. Fallback if it stalls: a short-lived env flag — explicitly a last resort, deleted within the arc. |
