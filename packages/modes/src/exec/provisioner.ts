...
export interface BuildSpawnSpecOpts {
	...
	thinking?: string;
	/**
	 * When set, pi runs under a shell wrapper that redirects the child's
	 * stdout+stderr to this file, so a crashing worker's dying screen (stack
	 * trace, provider error, OOM message) survives instead of the poll only
	 * ever seeing "session gone".
	 */
	crashFile?: string;
}
...
export function buildSpawnSpec(opts: BuildSpawnSpecOpts): SpawnSpec {
	...
	// Crash capture: run pi under a shell wrapper that redirects the child's
	// stdout+stderr straight to the crash file, so its dying screen (stack
	// trace, provider error, OOM message) survives. Running under a shell also
